...
class User {
    ...
    public:
        ...
        inline Gender getGender() const         { return this->gender; }
...
inline ostream& operator<<(ostream& flujo, const User& user){
    ...
    return flujo << user.getName() << " ("
                 << user.getEmail() << ") " << endl
                 << user.getBirthdate() << " | "
                 << user.getGender() << endl << "Canciones guardadas: "
                 << user.getNumSavedSongs() << " | Playlists creadas: "
                 << user.getNumPlaylists() << endl;
